...
def edit_in_progress() -> str:
    return "⏳ Применяю правку, это займёт до минуты…"
...
def help_text() -> str:
    return (
        ...
        "2. После правки бот в течение минуты присылает новую версию. "
        "Можно править сколько угодно раз.\n\n"
        "3. Голосовые сообщения распознаются автоматически.\n\n"
        "Команды:\n"
        "/pending - показать статьи, ожидающие ревью\n"
        "/plan - показать план публикаций (сколько и каких статей в день)\n"
        "/setplan - изменить план, напр. /setplan 3 3 3 1\n"
        "/help - эта справка"
    )
