...
7. маткапитал/маткап → материнский капитал (по падежам)
8. Точка без пробела перед заглавной буквой: «слово.Слово» → «слово. Слово»
9. Де-ёфикация: ё→е, Ё→Е во всём видимом тексте (text.ru-корректор подсвечивает
   несогласованную ёфикацию как ошибки правописания; единообразное «е» убирает класс).
10. Расцепление дефисных сцепок субъектов (кредитор-поставщик, организация-должник,
    Покупатель-банкрот, Кредитор-юридическое лицо). text.ru метит такие новообразования
    как ошибки. Правило: «роль-тип лица» → роль; «роль-роль» → вторая (уточняющая) часть.
    Срабатывает ТОЛЬКО когда обе части — юр-роли из словаря, поэтому термины
    (счёт-фактура, мастер-франшиза) и географию (Западно-Сибирский) не трогает.
...
REPLACEMENTS = [
    ...
    # маткапитал (материнский капитал, м.р.)
    (
        re.compile(r"\bматкапитал(а|у|ом|е)?\b", re.IGNORECASE | re.UNICODE),
        {
            "":   "материнский капитал",
            "а":  "материнского капитала",
            "у":  "материнскому капиталу",
            "ом": "материнским капиталом",
            "е":  "материнском капитале",
        },
    ),
    # маткап (короткое) → материнский капитал
    (
        re.compile(r"\bматкап\b", re.IGNORECASE | re.UNICODE),
        {"": "материнский капитал"},
    ),
    # заемовладелец — выдуманное слово (такого в русском нет) → заёмщик.
    # Пишем «заемщик» (без ё) для согласования с де-ёфикацией, которая
    # выполняется раньше в конвейере fix_text.
    (
        re.compile(r"\bза[её]мовладел(ьцами|ьцах|ьцам|ьцев|ьцем|ьца|ьцу|ьце|ьцы|ец)\b",
                   re.IGNORECASE | re.UNICODE),
        {
            "ец":    "заемщик",     # им. падеж — без мягкого знака (владелЕЦ)
            "ьца":   "заемщика",
            "ьцу":   "заемщику",
            "ьцем":  "заемщиком",
            "ьце":   "заемщике",
...
# С конца мая 2026 writer/seo-editor, дробя топ-леммы под анти-спам, начали
# лепить новообразования «кредитор-поставщик», «организация-должник»,
# «Покупатель-банкрот», «Кредитор-юридическое лицо». text.ru-корректор метит
# их как ошибки правописания (несуществующее слово / неверный дефис). Это и
# был источник всплеска «ошибок» в проверках. Расцепляем детерминированно.
#
# Срабатывает СТРОГО когда ОБЕ части — юр-роли из словаря (или роль + тип
# лица). Поэтому термины (счёт-фактура, мастер-франшиза, дизайн-макет) и
# географию (Западно-Сибирский) не трогает — там вторая часть не роль.
#
# Корни даны в «е»-форме: де-ёфикация выполняется РАНЬШЕ (партнёр→партнер,
# заёмщик→заемщик), поэтому матчим уже «партнер», «заемщик».
_ROLE_STEMS = (
    "кредитор", "должник", "банкрот", "контрагент", "поставщик", "покупател",
    "заказчик", "подрядчик", "партнер", "заемщик", "взыскател", "заявител",
    "залогодател", "залогодержател", "поручител", "арендатор", "арендодател",
    "наследник", "дольщик", "застройщик", "страховател", "страховщик",
    "кооператив", "акционер", "учредител", "участник", "пайщик", "инвестор",
    "вкладчик", "гражданин", "организаци", "компани", "фирм", "предприяти",
    "налогоплательщик",
)
# Тип лица: вторая часть сцепки «роль-тип» (Кредитор-юридическое лицо).
_TYPE_STEMS = ("юридическ", "физическ", "юрлиц", "физлиц", "должностн")

# Длинные корни первыми, чтобы alternation не обрывалась на коротком префиксе.
_ROLE_ALT = "|".join(sorted(_ROLE_STEMS, key=len, reverse=True))
_TYPE_ALT = "|".join(sorted(_TYPE_STEMS, key=len, reverse=True))

# «роль-тип(лица)» → роль. Опционально поглощаем следующее слово «лиц…»
# (для «юридическое лицо», «физических лиц»).
HYPHEN_ROLE_TYPE_RX = re.compile(
    r"\b((?:" + _ROLE_ALT + r")[а-яё]*)-(?:" + _TYPE_ALT + r")[а-яё]*(?:\s+лиц[а-яё]*)?",
    re.IGNORECASE | re.UNICODE,
)
# «роль-роль» → вторая (уточняющая) часть. Обе части согласованы по падежу,
# поэтому правая часть уже в нужной форме.
HYPHEN_ROLE_ROLE_RX = re.compile(
    r"\b((?:" + _ROLE_ALT + r")[а-яё]*)-((?:" + _ROLE_ALT + r")[а-яё]*)\b",
    re.IGNORECASE | re.UNICODE,
)


def _keep_case(original: str, kept: str) -> str:
    """Если исходная сцепка начиналась с заглавной — капитализируем результат."""
    if original[:1].isupper():
        return kept[:1].upper() + kept[1:]
    return kept


def decouple_hyphen_compounds(text: str) -> tuple[str, int]:
    """Расцепляет дефисные сцепки субъектов. Возвращает (текст, число замен)."""
    count = 0

    def repl_type(m: re.Match) -> str:
        nonlocal count
        count += 1
        return _keep_case(m.group(0), m.group(1))

    def repl_role(m: re.Match) -> str:
        nonlocal count
        count += 1
        return _keep_case(m.group(0), m.group(2))

    # Сначала «роль-тип» (поглощает хвост «лицо»), затем «роль-роль».
    text = HYPHEN_ROLE_TYPE_RX.sub(repl_type, text)
    text = HYPHEN_ROLE_ROLE_RX.sub(repl_role, text)
    return text, count


def deyoficate(text: str) -> tuple[str, int]:
    """ё→е, Ё→Е. Возвращает (текст, число замен)."""
    n = text.count("ё") + text.count("Ё")
    if n:
...
class FixReport:
    ...
    yo_fixes: int = 0          # де-ёфикация ё→е
    hyphen_fixes: int = 0      # расцепление дефисных сцепок субъектов
    changed: bool = False
    bytes_before: int = 0
    bytes_after: int = 0
...
def fix_text(text: str, is_html: bool) -> tuple[str, FixReport]:
    ...
    # Де-ёфикация (раньше всего: дефис-расцепление матчит «е»-корни)
    text, n_yo = deyoficate(text)
    rep.yo_fixes = n_yo

    # Расцепление дефисных сцепок субъектов (до раскрытия сокращений, чтобы
    # «кредитор-юрлица» обработать целиком как «роль-тип»)
    text, n_hyphen = decouple_hyphen_compounds(text)
    rep.hyphen_fixes = n_hyphen
...
def main() -> int:
    ...
    if args.json:
        ...
        if rep.yo_fixes:
            print(f"Де-ёфикация (ё → е): {rep.yo_fixes}")
        if rep.hyphen_fixes:
            print(f"Расцеплено дефисных сцепок субъектов: {rep.hyphen_fixes}")
        if not rep.changed:
            print("Изменений нет.")
