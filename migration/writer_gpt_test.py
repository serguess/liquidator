...
def build_user_prompt(slug: str, category: str) -> str:
    ...
    instr = (
        ...
        "6. ПОРЯДОК КОНЦОВКИ строго такой (частая ошибка — перепутать): "
        "последний смысловой H2-блок → `<!--BP:CTA-BOTTOM-->` → `<!--BP:DISCLAIMER-->` "
        "→ `## Частые вопросы` (FAQ: H2 + H3-вопросы с ответами) → строка копирайта. "
        "CTA-BOTTOM и дисклеймер идут ДО блока FAQ, НЕ после. FAQ — последний смысловой "
        "раздел статьи (после него только копирайт). НЕ ставь CTA или дисклеймер после FAQ.\n\n"
    )
