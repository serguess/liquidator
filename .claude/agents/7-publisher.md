...
НЕТ. Не нужно. Этот агент специально сделан тонким. Любое расширение функциональности — отдельная задача, которая требует обновления промпта. Если ты в процессе работы видишь ситуацию вроде «о, надо бы сразу скопировать в articles/, чтоб ускорить» — НЕ делай.
