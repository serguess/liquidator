...
Если опять failed → можно повторить /rewrite-article ещё раз, но **максимум 3 итерации** (считается из `_pipeline.log.json -> current_iteration`). После третьей — статья уходит в `drafts/_review/` и требует ручного разбора.
