...
## Если открываете в первый раз - 2 минуты на чтение

Удобнее всего смотреть темы не здесь, а в браузере: **`/preview/topics`** на сайте. Там визуально, с фильтрами и легендой по тегам.

**Что от вас нужно:** на странице `/preview/topics` пройдитесь по 32 темам и для каждой решите - идёт в работу или нет. Чтобы одобрить тему, откройте её файл на GitHub (кнопка «Редактировать» возле темы), смените `"status": "proposed"` на `"approved"`, нажмите «Commit changes». Можно одобрять не все сразу - начните с 5-7 приоритетных, остальные пометите позже.

Когда наберётся 5-7 одобренных тем, мы запустим конвейер на них и через несколько часов вы увидите готовые статьи в превью на проверку.
...
- ~~`news.json`~~ - новости (категория временно отключена, не входит в текущий объём работ)
